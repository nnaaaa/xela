    ...
    const config = useMemo<ApexOptions>(() => ({
            ...
            yaxis: [
                ...
                },
                {
                    ...
                    label: {
                        ...
                        text: "Sell Avg. Price @ " + sellAvgPrice.toFixed(2)
                    }
                }
            ],
        ...
        tooltip: {
            ...
            custom: function (opts) {
                ...
                let text = "<div class='font-mono p-2 bg-background text-foreground border-foreground'>";
                text += "Price : " + open + "<br>";
                // text += "High : " + high + "<br>";
                // text += "Low : " + low + "<br>";
                // text += "Close : " + close;

                const time = moment(opts.ctx.w.config.series[opts.seriesIndex].data[opts.dataPointIndex].x).format("DD MMM");
                ...
                for (const trade of trades) {
                    ...
                    text += "<br>";
                    text += trade.isBuyer ? "<span class='text-chart-2'>" : "<span class='text-chart-5'>";
                    text += trade.isBuyer ? "Buy" : "Sell";
                    text += "<span class='text-muted-foreground'>"
                    text += " at " + moment(trade.time).format("DD MMM YYYY HH:mm");
                    text += "</span>";
                    text += " : " + trade.qty;
                    text += " @ " + trade.price;
                    text += "</span>";
