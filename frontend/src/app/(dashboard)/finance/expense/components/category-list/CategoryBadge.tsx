...
export function CategoryBadge({category, className, targetAmount}: IProps) {
    const percent = useMemo(() => {
        ...
        const totalAmount = category.totalSpentAmounts.reduce((acc, curr) => acc + Math.abs(curr.amount), 0);
        if (Math.abs(totalAmount) > targetAmount) return 100;
        return Math.abs((totalAmount / targetAmount) * 100);
    }, [category.totalSpentAmounts, targetAmount]);
