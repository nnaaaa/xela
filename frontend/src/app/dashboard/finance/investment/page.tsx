...
import {GetCryptoProfileQuery, QueryGetCryptoProfilesArgs} from "@/gql/graphql";
...
function InvestmentPage({profiles, refresh}: IProps) {
    const [selectedCryptoProfile, setSelectedCryptoProfile] = useState(profiles?.[0])
            ...
            {selectedCryptoProfile && (
                <div className="flex gap-4">
                    <div className="flex flex-1 flex-col gap-4">
                        ...
                        >
                            <InvestmentSummary profile={selectedCryptoProfile}/>
                        </div>
                        <HistoricalBalanceChart historicalData={selectedCryptoProfile?.historicalBalances}/>
                    </div>
                    <div
                        className="flex-1 flex py-6 px-6 gap-1 rounded-lg border shadow-sm"
                    >
...
export default function InvestmentContainer() {
    ...
    const {
        ...
        loading,
        refetch,
    } = useQuery<GetCryptoProfileQuery, QueryGetCryptoProfilesArgs>(GET_CRYPTO_PROFILES, {variables: {data: {userId: Number(user?.id)}}})
    ...
    return <InvestmentPage profiles={data.getCryptoProfiles} refresh={refresh}/>
}